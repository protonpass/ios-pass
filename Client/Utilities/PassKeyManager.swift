...
public protocol PassKeyManagerProtocol: AnyObject {
    ...
    func getShareKey(shareId: String, keyRotation: Int64) async throws -> Data

    /// Get the latest key of an item to encrypt item content
    func getLatestItemKey(shareId: String, itemId: String) async throws -> Data
}

struct DecryptedShareKey {
    let shareId: String
    let keyRotation: Int64
    let keyData: Data
}

struct DecryptedItemKey {
    let shareId: String
    let itemId: String
    let keyRotation: Int64
    let keyData: Data
...
extension PassKeyManager: PassKeyManagerProtocol {
    public func getShareKey(shareId: String, keyRotation: Int64) async throws -> Data {
        ...
        if let decryptedShareKey =
            ...
            return decryptedShareKey.keyData
        }

        logger.trace("No cached share key \(keyDescription). Getting from repository")
        let encryptedShareKey = try await getEncryptedShareKey(shareId: shareId,
                                                               keyRotation: keyRotation)

        logger.trace("Decrypting share key \(keyDescription)")

        guard let encryptedKeyData = try encryptedShareKey.key.base64Decode() else {
        ...
        let decryptedKeyData = decryptedKey.content

        decryptedShareKeys.append(.init(shareId: shareId,
                                        keyRotation: keyRotation,
                                        keyData: decryptedKeyData))
        return decryptedKeyData
    }

    public func getLatestItemKey(shareId: String, itemId: String) async throws -> Data {
        let keyDescription = "shareId \"\(shareId)\", itemId: \"\(itemId)\""
        logger.trace("Getting latest item key \(keyDescription)")
        let latestItemKey = try await itemKeyDatasource.getLatestKey(shareId: shareId,
                                                                     itemId: itemId)
        ...
        if let decryptedItemKey =
            ...
            return decryptedItemKey.keyData
        }

        logger.trace("Decrypting latest item key \(keyDescription)")

        let vaultKey = try await getShareKey(shareId: shareId,
        ...
        decryptedItemKeys.append(.init(shareId: shareId,
                                       itemId: itemId,
                                       keyRotation: latestItemKey.keyRotation,
                                       keyData: decryptedItemKeyData))
        return decryptedItemKeyData
    }
}
