...
//        let allEncryptedShareKeys = try await shareKeyRepository.getKeys(userId: userId, shareId: shareId)
//        // The key with the biggest key rotation
...
public actor PassKeyManager: PassKeyManagerProtocol {
    ...
    private func getCachedKey(id: String, keyRotation: Int64) -> (any CryptographicKeyProtocol)? {
        keyCache[id]?[keyRotation]
    }

    private func getLatestCachedKey(id: String) -> (any CryptographicKeyProtocol)? {
    ...
    private func cacheKey(_ key: any CryptographicKeyProtocol, id: String) {
        keyCache[id, default: [:]][key.keyRotation] = key
    }

    // MARK: - Key Loading

    private func loadKeysIfNeeded() async throws {
    ...
    private func performKeyLoading() async throws {
        async let shareKeysRequest = shareKeyRepository.getAllLocalKeys()
        async let folderKeysRequest = folderKeyDatasource.getAllFolderKeys()
        let (shareKeys, folderKeys) = try await (shareKeysRequest, folderKeysRequest)
    ...
    private func decryptSymmetricKey(_ key: SymmetricallyEncryptedKeyType,
...
//        }
//
//        loadingTask = Task<Void, Error> { [weak self] in
//            guard let self else { throw PassError.deallocatedSelf }
//
//            async let shareKeysRequest = shareKeyRepository.getAllLocalKeys()
//            async let folderKeysRequest = folderKeyDatasource.getAllFolderKeys()
...
//            let symmetricKey = try await symmetricKeyProvider.getSymmetricKey()
//
//            for key in shareKeys as [SymmetricallyEncryptedKeyType] {
//                let decryptedKey = try await decryptSymmetricKey(key, using: symmetricKey)
//                await cacheKey(decryptedKey, id: key.id)
//            }
//
//            for key in folderKeys as [SymmetricallyEncryptedKeyType] {
//                let decryptedKey = try await decryptSymmetricKey(key, using: symmetricKey)
//                await cacheKey(decryptedKey, id: key.id)
//            }
//
...
//    private func decryptSymmetricKey(_ key: SymmetricallyEncryptedKeyType,
//                                     using symmetricKey: SymmetricKey) async throws -> any
//                                     CryptographicKeyProtocol {
//        let decryptedKey = try symmetricKey.decrypt(key.encryptedKey)
//        guard let decryptedKeyData = try decryptedKey.base64Decode() else {
//            throw PassError.crypto(.failedToBase64Decode)
//        }
...
public extension PassKeyManager {
    ...
    func getLatestItemKey(userId: String,
        ...
        logger.trace("Decrypting latest item key \(keyDescription)")
        let decryptedItemKey = try decryptItemKey(latestItemKey, parentId: containerId, itemId: itemId)
        logger.trace("Decrypted latest item key \(keyDescription)")
    ...
    func getItemKeys(userId: String,
        ...
        let decryptedKeys = try encryptedKeys.map { encryptedKey in
            try decryptItemKey(encryptedKey, parentId: containerId, itemId: itemId)
        }
    ...
    func getItemKey(userId: String,
        ...
        return try decryptItemKey(encryptedKey, parentId: containerId, itemId: itemId)
    }
...
//    -> [any CryptographicKeyProtocol] {
//        try await encryptedKeys.asyncMap { encryptedKey in
//            try await symmetricDecryptAndCache(encryptedKey)
//        }
//    }
