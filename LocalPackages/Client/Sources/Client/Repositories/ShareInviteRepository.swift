...
//        let userId = try await userManager.getActiveUserId()
//        return try await remoteDataSource.getInviteRecommendations(userId: userId, shareId: shareId, query:
//        query)
//    }
//
//    func checkAddresses(shareId: String, emails: [String]) async throws -> [String] {
...
//        return try await withThrowingTaskGroup(of: [String].self, returning: [String].self) { [weak self] group
//        in
//            guard let self else { return [] }
//            for batch in emails.chunked(into: 10) {
//                group.addTask {
//                    try await self.remoteDataSource.checkAddresses(userId: userId, shareId: shareId, emails:
//                    batch)
//                }
//            }
//
