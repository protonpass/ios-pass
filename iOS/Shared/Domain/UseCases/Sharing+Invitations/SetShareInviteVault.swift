...
protocol SetShareInviteVaultUseCase {
    func execute(with vault: SharingVault)
}

extension SetShareInviteVaultUseCase {
    func callAsFunction(with vault: SharingVault) {
        execute(with: vault)
    }
}
